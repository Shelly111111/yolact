...
class ToCV2Image(object):
    def __call__(self, tensor, masks=None, boxes=None, labels=None):
        return tensor.numpy().astype(np.float32).transpose((1, 2, 0)), masks, boxes, labels


class ToTensor(object):
    def __call__(self, cvimage, masks=None, boxes=None, labels=None):
        return paddle.to_tensor(cvimage.astype(np.float32)).permute(2, 0, 1), masks, boxes, labels
...
class FastBaseTransform(paddle.nn.Layer):
    """
    ...
    def __init__(self):
        ...
        self.mean = paddle.to_tensor(MEANS[None, :, None, None]).astype('float32')
        self.std  = paddle.to_tensor( STD[None, :, None, None] ).astype('float32')
