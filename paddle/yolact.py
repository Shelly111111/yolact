...
class ScriptModule(nn.Layer):  # type: ignore[no-redef]
        def __init__(self, arg=None):
            super().__init__()

ScriptModuleWrapper = ScriptModule if use_jit else nn.Layer
...
class PredictionModule(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels=1024, aspect_ratios=[[1]], scales=[1], parent=None, index=0):
        ...
        if parent is None:
            ...
            if cfg.use_prediction_module:
                ...
                self.conv = nn.Conv2D(out_channels, out_channels, kernel_size=1, bias_attr=True)
    ...
    def forward(self, x):
        ...
        bbox = src.bbox_layer(bbox_x).permute(0, 2, 3, 1).contiguous().reshape(x.size(0), -1, 4)
        conf = src.conf_layer(conf_x).permute(0, 2, 3, 1).contiguous().reshape(x.size(0), -1, self.num_classes)
        
        if cfg.eval_mask_branch:
            mask = src.mask_layer(mask_x).permute(0, 2, 3, 1).contiguous().reshape(x.size(0), -1, self.mask_dim)
        else:
            mask = paddle.zeros(x.size(0), bbox.size(1), self.mask_dim)

        if cfg.use_mask_scoring:
            score = src.score_layer(x).permute(0, 2, 3, 1).contiguous().reshape(x.size(0), -1, 1)

        if cfg.use_instance_coeff:
            inst = src.inst_layer(x).permute(0, 2, 3, 1).contiguous().reshape(x.size(0), -1, cfg.num_instance_coeffs)    
        ...
        if cfg.eval_mask_branch:
            if cfg.mask_type == mask_type.direct:
                ...
                if cfg.mask_proto_coeff_gate:
                    gate = src.gate_layer(x).permute(0, 2, 3, 1).contiguous().reshape(x.size(0), -1, self.mask_dim)
        ...
        priors = self.make_priors(conv_h, conv_w, 0)
    ...
    def make_priors(self, conv_h, conv_w, device):
        ...
        with timer.env('makepriors'):
            if self.last_img_size != (cfg._tmp_img_w, cfg._tmp_img_h):
                ...
                self.priors = paddle.to_tensor(prior_data).reshape(-1, 4).detach()
...
class Yolact(nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if cfg.use_class_existence_loss:
                pred_outs['classes'] = self.class_existence_fc(outs[-1].mean(axis=(2, 3)))
            ...
            if cfg.use_focal_loss:
                if cfg.use_sigmoid_focal_loss:
                    ...
                    objectness = paddle.sigmoid(pred_outs['conf'][:, :, 0])
                ...
                if cfg.use_objectness_score:
                    ...
                    pred_outs['conf'][:, :, 1:] = (objectness > 0.10)[..., None] \
                        * F.softmax(pred_outs['conf'][:, :, 1:], axis=-1)
