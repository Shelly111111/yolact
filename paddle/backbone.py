...
class Bottleneck(nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_layer=nn.BatchNorm2D, dilation=1, use_dcn=False):
        ...
        self.conv1 = nn.Conv2D(inplanes, planes, kernel_size=1, bias_attr=False, dilation=dilation)
        ...
        self.conv3 = nn.Conv2D(planes, planes * 4, kernel_size=1, bias_attr=False, dilation=dilation)
...
class ResNetBackbone(nn.Layer):
    ...
    def __init__(self, layers, dcn_layers=[0, 0, 0, 0], dcn_interval=1, atrous_layers=[], block=Bottleneck, norm_layer=nn.BatchNorm2D):
        ...
        self.bn1 = norm_layer(64,momentum=0.1)
        self.relu = nn.ReLU()
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dcn_layers=0, dcn_interval=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsample = nn.Sequential(
                ...
                self.norm_layer(planes * block.expansion,momentum=0.1),
            )
...
class ResNetBackboneGN(ResNetBackbone):
    ...
    def init_backbone(self, path):
        ...
        for key in our_state_dict_keys:
            ...
            new_state_dict[key] = paddle.to_tensor(state_dict[transcribed_key])
...
def darknetconvlayer(in_channels, out_channels, *args, **kwdargs):
    ...
    return nn.Sequential(
        nn.Conv2D(in_channels, out_channels, *args, **kwdargs, bias_attr=False),
...
class DarkNetBackbone(nn.Layer):
    """
    ...
    def init_backbone(self, path):
        """ Initializes the backbone weights for training. """
        # Note: Using strict=False is berry scary. Triple check this.
        self.set_state_dict(paddle.load(path), strict=False)
...
class VGGBackbone(nn.Layer):
    ...
    def __init__(self, cfg, extra_args=[], norm_layers=[]):
        ...
        self.norms = nn.LayerList([nn.BatchNorm2D(self.channels[l],momentum=0.1) for l in norm_layers])
