...
def intersect(box_a, box_b):
    ...
    A = box_a.shape[1]
    B = box_b.shape[1]
    max_xy = paddle.minimum(paddle.to_tensor(box_a[:, :, 2:]).unsqueeze(2).expand(n, A, B, 2),
                       paddle.to_tensor(box_b[:, :, 2:]).unsqueeze(1).expand(n, A, B, 2))
    min_xy = paddle.maximum(paddle.to_tensor(box_a[:, :, :2]).unsqueeze(2).expand(n, A, B, 2),
                       paddle.to_tensor(box_b[:, :, :2]).unsqueeze(1).expand(n, A, B, 2))
...
def jaccard(box_a, box_b, iscrowd:bool=False):
    ...
    area_b = paddle.to_tensor((box_b[:, :, 2]-box_b[:, :, 0]) *
...
def mask_iou(masks_a, masks_b, iscrowd=False):
    ...
    area_a = paddle.sum(masks_a,axis=1).unsqueeze(1)
    area_b = paddle.sum(masks_b,axis=1).unsqueeze(0)
...
def elemwise_mask_iou(masks_a, masks_b):
    ...
    masks_a = masks_a.reshape(-1, masks_a.size(-1))
    masks_b = masks_b.reshape(-1, masks_b.size(-1))

    intersection = paddle.sum(masks_a * masks_b, axis=0)
    area_a = paddle.sum(masks_a,axis=0)
    area_b = paddle.sum(masks_b,axis=0)
...
def sanitize_coordinates(_x1, _x2, img_size:int, padding:int=0, cast:bool=True):
    ...
    """
    _x1 = paddle.to_tensor(_x1 * img_size)
    _x2 = paddle.to_tensor(_x2 * img_size)
...
def crop(masks, boxes, padding:int=1):
    ...
    crop_mask = masks_left.astype('float32') * masks_right.astype('float32') * masks_up.astype('float32') * masks_down.astype('float32')
...
def index2d(src, idx):
    ...
    offs = paddle.arange(idx.size(0))[:, None].expand_as(idx)
    idx  = idx + offs * idx.size(1)

    return src.reshape(-1)[idx.reshape(-1)].reshape(idx.size())
