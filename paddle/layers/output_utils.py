...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    if dets is None:
        return [paddle.Tensor()] * 4 # Warning, this is 4 copies of the same thing
    ...
    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
        ...
        masks = proto_data @ paddle.to_tensor(masks).t()
        ...
        masks = paddle.to_tensor(masks).permute(2, 0, 1).contiguous()

        if cfg.use_maskiou:
            with timer.env('maskiou_net'):                
                with paddle.no_grad():
                    maskiou_p = net.maskiou_net(masks.unsqueeze(1))
                    maskiou_p = paddle.gather(maskiou_p, axis=1, index=classes.unsqueeze(1)).squeeze(1)
                    if cfg.rescore_mask:
                        if cfg.rescore_bbox:
    ...
    if cfg.mask_type == mask_type.direct and cfg.eval_mask_branch:
        # Upscale masks
        full_masks = paddle.zeros(masks.size(0), h, w)
...
def display_lincomb(proto_data, masks):
    out_masks = paddle.matmul(proto_data, masks.t())
    ...
    for kdx in range(1):
        ...
        test = paddle.sum(proto_data, -1).numpy()
        ...
        for y in range(arr_h):
            for x in range(arr_w):
                ...
                if i == 0:
                    running_total = proto_data[:, :, idx[i]].numpy() * coeffs_sort[i]
                else:
                    running_total += proto_data[:, :, idx[i]].numpy() * coeffs_sort[i]
                ...
                arr_img[y*proto_h:(y+1)*proto_h, x*proto_w:(x+1)*proto_w] = (proto_data[:, :, idx[i]] / paddle.max(proto_data[:, :, idx[i]])).numpy() * coeffs_sort[i]
                arr_run[y*proto_h:(y+1)*proto_h, x*proto_w:(x+1)*proto_w] = (running_total_nonlin > 0.5).astype('float32')
        plt.imshow(arr_img)
        plt.show()
        # plt.imshow(arr_run)
        # plt.show()
