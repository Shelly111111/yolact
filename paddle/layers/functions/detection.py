...
class Detect(object):
    ...
    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, inst_data):
        ...
        scores = paddle.to_tensor(cur_scores.numpy()[:, keep])
        boxes = decoded_boxes.numpy()[keep, :]
        masks = mask_data.numpy()[batch_idx, keep, :]
    ...
    def cc_fast_nms(self, boxes, masks, scores, iou_threshold:float=0.5, top_k:int=200):
        ...
        scores, classes = scores.max(axis=0)
    ...
    def fast_nms(self, boxes, masks, scores, iou_threshold:float=0.5, top_k:int=200, second_threshold:bool=False):
        ...
        iou=paddle.triu(iou, diagonal=1)
        ...
        classes = paddle.to_tensor(paddle.arange(num_classes).numpy()[:, None]).expand_as(keep)
    ...
    def traditional_nms(self, boxes, masks, scores, iou_threshold=0.5, conf_thresh=0.05):
        ...
        for _cls in range(num_classes):
            ...
            idx = paddle.arange(cls_scores.size(0))
            ...
            preds = paddle.concat([boxes[conf_mask], cls_scores[:, None]], axis=1).numpy()
