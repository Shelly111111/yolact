...
def make_priors(conv_size, scales, aspect_ratios):
    ...
    return paddle.to_tensor(prior_data).reshape(-1, 4)
...
def compute_hits(bboxes, anchors, iou_threshold=0.5):
    ...
    perGTAnchorMax = paddle.max(ious, axis=1)
    
    return (perGTAnchorMax > iou_threshold)

def compute_recall(hits, base_hits):
    hits = (hits | base_hits).astype('float32')
    return paddle.sum(hits) / hits.size(0)
...
def step(x, x_func, bboxes, base_hits, optim_idx):
    ...
    return -float(compute_recall(compute_hits(bboxes, anchors), base_hits))
...
def optimize(full_bboxes, optim_idx, batch_size=5000):
    ...
    base_hits = compute_hits(bboxes, paddle.concat(anchor_base, axis=0))
