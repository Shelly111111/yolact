...
def mask_iou(mask1, mask2):
    ...
    """
    intersection = paddle.matmul(mask1, mask2.t())
    area1 = paddle.sum(mask1, axis=1).reshape(1, -1)
    area2 = paddle.sum(mask2, axis=1).reshape(1, -1)
...
proto_masks = paddle.to_tensor(proto_masks).permute(2, 0, 1).contiguous().unsqueeze(0)
...
ious = mask_iou(paddle.to_tensor(approximated_masks.reshape(-1, num_gt).T),
				paddle.to_tensor(gt_masks.reshape(-1, num_gt).T))
