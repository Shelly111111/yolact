...
if __name__ == '__main__':
    ...
    with paddle.no_grad():
        ...
        for i in range((bboxes_rel.size(0) // chunk_size) + 1):
            ...
            maxes, maxidx = paddle.max(ious, axis=1)

            perGTAnchorMax[start:end] = maxes
    

        hits = (perGTAnchorMax > 0.5).astype('float32')

        print('Total recall: %.2f' % (paddle.sum(hits) / hits.size(0) * 100))
        print()

        for i, metric in zip(range(3), ('small', 'medium', 'large')):
            _hits = hits[sizes == i]
            _size = (1 if _hits.size(0) == 0 else _hits.size(0))
            print(metric + ' recall: %.2f' % ((paddle.sum(_hits) / _size) * 100))
